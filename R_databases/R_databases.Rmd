...
```{r, echo = TRUE, eval = FALSE, comment = "", message = FALSE}
hvc 

```
```{r, comment = "", message = FALSE}
library(dplyr)
head(hvc_simple, n=3) %>% select(PAT_ID:PREV_ANY_DISCHARGE)
...
Calculations done on database server BEFORE loading in R

```{r, echo = TRUE, eval = FALSE}
con <- DBI::dbConnect(odbc::odbc(), "StgEpicClarityPHS", 
